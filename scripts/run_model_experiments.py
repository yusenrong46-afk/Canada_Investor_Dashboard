...
def _load_mart(warehouse_path: Path) -> tuple[pd.DataFrame, dict[str, str]]:
    ...
    columns = (
        ["property_observation_id", "market_id", "province_state", "target_name", TARGET_COLUMN, DATE_COLUMN]
        + NUMERIC_FEATURES
...
def _random_market_split(market_frame: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def _temporal_market_split(
    market_frame: pd.DataFrame,
    *,
    months: int = TEMPORAL_HOLDOUT_MONTHS,
) -> tuple[pd.DataFrame, pd.DataFrame, str] | None:
    if DATE_COLUMN not in market_frame.columns:
        return None

    dates = pd.to_datetime(market_frame[DATE_COLUMN], errors="coerce")
    dated_mask = dates.notna()
    if not dated_mask.any():
        return None

    dated = market_frame.loc[dated_mask].copy()
    dates = dates.loc[dated_mask]
    cutoff = dates.max() - pd.DateOffset(months=months)
    train_frame = dated.loc[dates < cutoff].reset_index(drop=True)
    holdout_frame = dated.loc[dates >= cutoff].reset_index(drop=True)
    if len(train_frame) < 20 or len(holdout_frame) < 5:
        return None

    undated_excluded = int((~dated_mask).sum())
    strategy = f"temporal holdout: last {months} months of {DATE_COLUMN}"
    if undated_excluded:
        strategy += f"; {undated_excluded} undated rows excluded"
    return train_frame, holdout_frame, strategy


def _market_splits(frame: pd.DataFrame) -> tuple[dict[str, tuple[pd.DataFrame, pd.DataFrame]], dict[str, str]]:
    ...
    for market_id, market_frame in frame.groupby("market_id"):
        ...
        temporal = _temporal_market_split(market_frame)
        if temporal is not None:
            train_frame, holdout_frame, strategy = temporal
            splits[market_key] = (train_frame, holdout_frame)
            strategies[market_key] = strategy
            continue

        train_frame, holdout_frame = _random_market_split(market_frame)
        splits[market_key] = (train_frame, holdout_frame)
        strategies[market_key] = f"random 80/20 holdout (random_state {RANDOM_STATE}; no usable {DATE_COLUMN})"

    return splits, strategies


def _random_secondary_metrics(
    family: str,
    splits: dict[str, tuple[pd.DataFrame, pd.DataFrame]],
    frame: pd.DataFrame,
    ...
    for market_id, market_frame in frame.groupby("market_id"):
        market_key = str(market_id)
        random_train, random_holdout = _random_market_split(market_frame)
        pipeline = _make_pipeline(family, LOCAL_CATEGORICAL_FEATURES)
        pipeline.fit(random_train, random_train[TARGET_COLUMN])
        random_log_pred = np.asarray(pipeline.predict(random_holdout), dtype=float)
        metrics[market_key] = _price_space_metrics(
            random_holdout[TARGET_COLUMN].to_numpy(dtype=float),
            random_log_pred,
        )
    return metrics
...
def _market_row_notes(
    ...
    splits: dict[str, tuple[pd.DataFrame, pd.DataFrame]],
    strategies: dict[str, str],
    random_secondary: dict[str, tuple[float, float]],
    skipped: dict[str, list[tuple[str, int]]],
    spatial_mae: float | None,
) -> str | None:
    train_frame, holdout_frame = splits[market]
    pieces: list[str] = [f"Primary holdout: {strategies.get(market, 'unknown')}."]
    random_mae, random_mape = random_secondary.get(market, (None, None))
    if random_mae is not None and strategies.get(market, "").startswith("temporal"):
        pieces.append(f"Secondary random 80/20 holdout MAE ${random_mae:,.0f} (MAPE {random_mape:.1%}).")
            ...
            rows.append(
                {
                    ...
                    "notes": _market_row_notes(experiment, market, markets, splits, strategies, random_secondary, skipped, spatial_mae),
                }
            )
...
def _write_report(
    ...
    lines = [
        ...
        f"- Primary holdout per market: temporal last {TEMPORAL_HOLDOUT_MONTHS} months of `{DATE_COLUMN}` when dates "
        f"exist (undated rows excluded); otherwise random 80/20 (random_state {RANDOM_STATE}).",
...
def run_model_experiments(
    *,
    ...
    splits, strategies = _market_splits(frame)
    random_secondary = _random_secondary_metrics(family, splits, frame)
    ...
    rows = _build_rows(family, splits, strategies, random_secondary, predictions, training_rows, pooled_train, spatial, skipped)
    ...
    payload: dict[str, Any] = {
        ...
        "source": (
            ...
            f"experiments ({family}); per-market temporal holdout (last {TEMPORAL_HOLDOUT_MONTHS} months of "
            f"{DATE_COLUMN} when available, else random 80/20) with secondary random holdout metrics; spatial "
            "GroupKFold by postal FSA on training splits; MAE/MAPE in price space, MAPE as a fraction"
        ),
