...
def _method_text(baseline_month: str, control_median: float) -> str:
    return (
        ...
        "(sanity-checked within +/-10% of 1.0). Co-occurrence of Renovation and Addition between "
        "the same sales is unvalidated and not summed. Spatial permit matching leaves a large "
        "unmatched share, so treated pairs are a selected subset of geolocatable permits."
    )
...
def build_halifax_uplift_export(
    *,
    ...
    permit_match_rate = (len(matched_permits) / len(permits)) if len(permits) else 0.0
    export = {
        ...
        "permitMatchRate": round(permit_match_rate, 4),
        "coOccurrenceNote": (
            "Renovation and Addition co-occurrence is unvalidated; when both are ready the API "
            "serves the single dominant category instead of summing them."
        ),
        "selectionBiasNote": (
            "Permit→dwelling spatial match is incomplete within 30 m; unmatched permits never "
            "enter treated sets, so measured uplift is conditional on geolocatable permits and "
            "may over-represent well-matched parcels."
        ),
