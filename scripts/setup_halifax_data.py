...
def _fit_submarket_clusters(frame: pd.DataFrame) -> pd.Series:
    from sklearn.cluster import KMeans

    clusterer = KMeans(n_clusters=min(SUBMARKET_CLUSTERS, len(frame)), random_state=42, n_init=10)
    labels = clusterer.fit_predict(frame[["latitude", "longitude"]].to_numpy())
    return pd.Series([f"cluster-{label + 1:02d}" for label in labels], index=frame.index)
...
def build_training_extract(
    *,
    ...
    median_bedrooms = joined.groupby("propertyType")["bedrooms"].transform("median")
    joined["bedrooms"] = joined["bedrooms"].fillna(median_bedrooms).fillna(joined["bedrooms"].median())
    joined["bedroomsImputed"] = bedrooms_missing

    quality_mask = (
        joined["price"].between(100_000, 10_000_000)
        & joined["livingAreaSqft"].between(250, 10_000)
        & joined["bedrooms"].between(0, 10)
        & joined["bathrooms"].between(0, 10)
    )
    ...
    clean["submarketCluster"] = _fit_submarket_clusters(clean)
    clean["h3Cell"] = [
        _h3_cell(latitude, longitude) for latitude, longitude in zip(clean["latitude"], clean["longitude"])
    ]

    current_year = pd.Timestamp.now().year
    clean["ageYears"] = (current_year - clean["yearBuilt"]).clip(lower=0)
    ...
    summary = {
        ...
        "rates": {
            ...
            "bedroomsImputed": round(float(extract["bedroomsImputed"].mean()), 4),
