...
- Halifax sale prices include unobserved condition/renovation effects at sale time. The shipped trainer still builds its target from the extract's full-sample time adjustment, and it still trains on assessed value when that column is present. Inference nulls assessed value. Those shipped-path risks remain. `scripts/halifax_serving_evaluation.py` is a separate measurement: the price index uses only pre-holdout sales, the last six calendar months are the test slice, the six months before that are used only for a residual interval, and the features omit assessed value. On the retained extract (`data/processed/halifax_base_model_training.csv`, 17,702 rows) on 2026-09-26, the test slice starts 2025-12-01 (1,321 rows). A per-type median baseline has MAE $183,316 and MAPE 30.8%. A Ridge baseline has MAE $101,464 and MAPE 17.0%. The Ridge absolute-residual interval, set on 2,658 calibration rows, covers 76.7% of the test rows. The price index contains none of the holdout months. The measurement writes those metrics only, and the shipped file remains `halifax_base_price_bundle_v1.pkl`.
