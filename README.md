...
## What the live demo is (read this first)

**Production on Vercel runs public interactive mode** — a transparent TypeScript rules/screening engine with provenance labels (`rules` / `assumption` or `observed` / `unvalidated` or `descriptive`). It does **not** serve the fitted XGBoost/Random Forest pickles.

| Mode | Where | What calculates |
|---|---|---|
| **Public** (hosted demo) | Vercel | TypeScript screening rules + committed evidence exports |
| **Live** (local only) | Express + Flask | Fitted Vancouver/Halifax base models + uplift layers |
| **Demo** | `DEMO_MODE=true` | Precomputed sample JSON |

Live local mode is where conformal intervals, SHAP drivers, and temporal-holdout metrics apply. The public link is the truthful portfolio demo you can click without private data.
...
React / TypeScript frontend (all routes lazy-loaded)
  ...
  /deal-analyzer     One-page deal screening with assumption-based stress testing
...
- Deal robustness is a seeded triangular stress test over stated ranges — an assumption-based transparency tool, not a calibrated probability or market simulation.
