...
- Caveat: the two markets have different target semantics (Vancouver models listing price; Halifax (HRM) models time-adjusted sale price). Pooling tests whether cross-market structure transfers despite that difference, not that the targets are interchangeable.
