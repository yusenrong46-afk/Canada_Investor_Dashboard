...
def test_build_training_extract_joins_and_normalizes(tmp_path: Path):
    ...
    # Missing bedrooms are left null for the train-fold imputer (no full-data median fill).
    townhouse = extract[extract["propertyType"] == "Townhouse"].iloc[0]
    assert bool(townhouse["bedroomsImputed"]) is True
    assert pd.isna(townhouse["bedrooms"])
    assert (extract["submarketCluster"] == "unassigned").all()

    assert summary["targetName"] == "sale_price_time_adjusted"
    assert summary["rows"]["cleanTrainingRows"] == 3
    assert summary["cleaningPolicy"]["bedroomsMissing"] == "left-null-for-train-fold-imputer"
