...
def test_flags_map_and_dedupe_to_one_renovation_category(tmp_path, monkeypatch, fake_base_estimate) -> None:
    ...
    assert result["treatedQuantileRange"]["lowPercent"] == pytest.approx(-0.004)
    assert result["treatedQuantileRange"]["highPercent"] == pytest.approx(0.56)
...
def test_ready_categories_serve_dominant_category_not_sum(tmp_path, monkeypatch, fake_base_estimate) -> None:
    ...
    assert result["upliftPercent"] == pytest.approx(0.2)
    assert result["treatedQuantileRange"]["lowPercent"] == pytest.approx(0.05)
    assert result["treatedQuantileRange"]["highPercent"] == pytest.approx(0.4)
    assert result["upliftValue"] == round(BASE_VALUE * 0.2)
    assert {driver["upliftPercent"] for driver in result["topUpliftDrivers"]} == {0.2}
    assert any("dominant ready category" in note for note in result["methodNotes"])
