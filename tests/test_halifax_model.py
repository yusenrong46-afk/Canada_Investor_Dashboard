...
def test_unobserved_fsa_is_rejected_instead_of_using_market_centroid() -> None:
    with pytest.raises(ValueError, match="outside the Halifax model's observed training geography"):
        halifax_core._normalize_request(_payload(postalCode="B4B 0M9"), _tiny_bundle())
...
def test_estimate_property_returns_conformal_halifax_estimate(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    pytest.importorskip("sklearn")

    monkeypatch.setattr(halifax_core, "ARTIFACT_DIR", tmp_path)
    monkeypatch.setattr(halifax_core, "ARTIFACT_PATH", tmp_path / "halifax_base_price_bundle_v1.pkl")
    monkeypatch.setattr(halifax_core, "_BUNDLE", halifax_core.train_bundle(data_path=str(TRAINING_CSV)))
...
def test_load_bundle_rejects_missing_artifact_without_training(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    from common.artifact_loader import ModelArtifactError

    monkeypatch.setattr(halifax_core, "ARTIFACT_PATH", tmp_path / "missing.pkl")
    monkeypatch.setattr(halifax_core, "_BUNDLE", None)
    monkeypatch.setattr(
        halifax_core,
        "train_bundle",
