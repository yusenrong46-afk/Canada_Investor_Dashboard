...
describe("Halifax public simulate with the local uplift export", () => {
  it("sums observed medians once per distinct category and zeroes insufficient ones", async () => {
    ...
    expect(response.treatedQuantileRange?.lowPercent).toBe(0.05);
    expect(response.treatedQuantileRange?.highPercent).toBe(0.21);
    ...
    expect(response.treatedQuantileRange?.lowValue).toBe(Math.round(baseValue * 0.05));
    expect(response.treatedQuantileRange?.highValue).toBe(Math.round(baseValue * 0.21));
  ...
  it("costs committed scope once per checkbox while keeping the broad renovation uplift non-additive", async () => {
    const { buildPublicPlan } = await importPublicEngineWithUplift(upliftFixture);
    ...
    expect(response.status).toBe("ready");
    expect(response.plannedSpend).toBe(100_000);
    expect(response.items).toHaveLength(2);
    expect(response.items?.every((item) => item.origin === "committed")).toBe(true);
    expect(response.items?.[0].projectedUplift).toBeGreaterThan(0);
    expect(response.items?.[1].projectedUplift).toBe(0);
    expect(response.methodNotes?.join(" ")).toContain("above the 85,000 dollar budget");
    expect(response.upliftConfidenceLow).toBeGreaterThan(0);
  });

  it("abstains instead of substituting generic Halifax uplift when the export is absent", async () => {
    const { buildPublicSimulate } = await importPublicEngineWithUplift(null);

    const response = buildPublicSimulate({ ...halifaxProperty, plannedFlags: ["renovatedKitchen"] });

    expect(response.status).toBe("data-missing");
    expect(response.dataSources?.halifaxUplift).toBeUndefined();
    expect(response.upliftValue).toBeUndefined();
    expect(response.message).toContain("No generic uplift assumption was substituted");
  });

  it("abstains when the export exists but no category is ready", async () => {
