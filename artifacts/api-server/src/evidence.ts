...
import { marketEvidenceFileSchema, marketValues, type MarketEvidenceFile, type MarketEvidenceRow, type MarketId } from "@vvl/shared";
...
export function loadMarketEvidence(): MarketEvidenceFile | null {
  if (cachedEvidence === undefined) {
    try {
      cachedEvidence = marketEvidenceFileSchema.parse(readRepoJson<unknown>(evidenceRelativePath));
    } catch {
      cachedEvidence = null;
    }
  }
