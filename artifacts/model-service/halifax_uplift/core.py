...
def _zero_uplift_response(base_estimate: dict[str, Any], export: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "topUpliftDrivers": [],
        "observedShare": 1.0,
        "methodNotes": [export["method"], DEDUPE_NOTE],
    }
...
def simulate_uplift(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    uplift_percent = float(sum(categories[category]["medianExcessUpliftPercent"] for category in ready))
    low_percent = float(sum(categories[category]["p25ExcessUpliftPercent"] for category in ready))
    high_percent = float(sum(categories[category]["p75ExcessUpliftPercent"] for category in ready))
    ...
    method_notes = [export["method"], DEDUPE_NOTE]
    if insufficient:
        skipped = "; ".join(
    ...
    return {
        ...
        "upliftConfidenceLow": int(round(base_value * low_percent)),
        "upliftConfidenceHigh": int(round(base_value * high_percent)),
