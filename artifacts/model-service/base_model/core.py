...
def _train_property_type_model(property_type: str, frame: pd.DataFrame) -> tuple[Pipeline, str, dict[str, dict[str, Any]], ConformalCalibration, dict[str, Any]]:
    ...
    # Split conformal replaces the old 70th-percentile error-ratio band; ratio is not clipped
    # because clipping would void the finite-sample coverage guarantee.
    ...
    # Spatial CV: GroupKFold by postal FSA on the already-selected family only, so leakage
    # from nearby listings in the same FSA is measured without re-running model selection.
    train_fsa_groups = frame.loc[train_index, "postalFsa"]
    ...
    if distinct_fsas >= 2:
        ...
        for spatial_train_pos, spatial_valid_pos in spatial_splitter.split(x_train, groups=train_fsa_groups):
            ...
            spatial_actual = frame.loc[spatial_valid_index, "price"].to_numpy(dtype=float)
