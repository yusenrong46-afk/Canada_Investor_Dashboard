...
export function MarketTrendCard({ market }: MarketTrendCardProps) {
  ...
  useEffect(() => {
    ...
    getMarketTrend(market, controller.signal)
      .then((response) => {
        setTrend(response);
      })
      .catch(() => {
      ...
      <div className="h-72">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
            ...
            <XAxis dataKey="period" minTickGap={48} tickLine={false} axisLine={false} tick={{ fill: chartTickFill, fontSize: 12 }} />
            <YAxis
              domain={["auto", "auto"]}
              width={56}
              tickLine={false}
            ...
            />
            <Area type="monotone" dataKey="forecastBand" name="Forecast interval" stroke="none" fill={chartBarSecondary} fillOpacity={0.15} />
            <Line type="monotone" dataKey="indexValue" name="Index" stroke={chartBarSecondary} strokeWidth={2} dot={false} />
            <Line type="monotone" dataKey="forecastValue" name="Forecast" stroke={chartBarPrimary} strokeWidth={2} strokeDasharray="6 4" dot={false} />
            <ReferenceLine
              x={ready.baselinePeriod}
              stroke={chartTickFill}
              strokeDasharray="4 4"
              label={{ value: "latest data", fill: chartTickFill, fontSize: 11, position: "insideTopLeft" }}
            />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
